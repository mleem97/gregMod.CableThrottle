# AGENTS.md — Notes for AI agents (gregMod.CableThrottle)

Repo: https://github.com/mleem97/gregMod.CableThrottle · License: Apache-2.0 · Version: see `VERSION` (1.4.0).

MelonMod for Data Center. Cable throughput throttling.
...
4. **Verify changes:** before reporting done, build the mod (`dotnet build gregMod.CableThrottle.csproj -c Release` or `./build.sh CableThrottle` from `ModRepositories/`).
...
- Deploy only with `./build.sh CableThrottle --deploy`.

## Hard rules

- Throttling is observational plus the minimal rate override — never suppress
  vanilla cable behaviour beyond the throttled rate.
...
- `src/CableThrottleMod.cs` — MelonMod entry, prefs, toggle.
